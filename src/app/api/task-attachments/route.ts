...
export async function GET(req: NextRequest) {
  ...
  const { data: rows, error } = await admin
    ...
    .select('id, type, display_name, file_name, file_size, mime_type, storage_path, url, link_title, link_domain, link_favicon_url, created_at, uploaded_by, is_internal, client_visible')
    .eq('task_id', taskId)
    .eq('company_id', companyId)
